...
function showStatus(kind, title, message) {
  ...
  $("categories").innerHTML = `${diyLaunchCard()}<article class="status-card ${kind}">
...
function bindDiyLaunchers() {
  document.querySelectorAll("[data-action='open-diy']").forEach((button) => {
    button.addEventListener("click", openDiy);
  });
}
...
function createLayerCanvas() {
  ...
  canvas.width = DIY_WIDTH;
  canvas.height = DIY_HEIGHT;
  return canvas;
}
...
function drawLine(from, to) {
  ...
  ctx.lineWidth = Number($("diy-brush").value);
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ctx.beginPath();
...
function buildMaskDownload(kind) {
  ...
  const source = kind === "light" ? diy.light : diy.mask;
